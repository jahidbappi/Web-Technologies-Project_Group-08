...
/** Allow redirect back to task3 (or other app paths) after login — blocks open redirects. */
function auth_safe_return_url(?string $return): ?string {
    if ($return === null || $return === '') {
        return null;
    }
    $return = str_replace('\\', '/', $return);
    if ($return[0] !== '/' || str_contains($return, '..') || preg_match('/[\r\n]/', $return)) {
        return null;
    }
    $base = app_base_path();
